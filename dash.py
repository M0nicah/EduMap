...
if critical_only:
    filtered_df = filtered_df[filtered_df['Extra_Teachers_Required'] >= 15]

# Priority score
filtered_df['Priority'] = filtered_df['Extra_Teachers_Required'] + filtered_df['extra_toilets_required']

# CREATE TABS
tab1, tab2 = st.tabs(["📊 National Stats", "⚖️ Comparison Mode"])

# TAB 1: NATIONAL STATS
with tab1:
    ...
    national_avg_ratio = filtered_df['Students_per_Teacher'].mean() if not filtered_df.empty else 0
    ...
    with col1:
        ...
        # Insight box
        if 'PUBLIC' in private_vs_public['Status'].values:
    ...
    if not district_shortage.empty:
    ...
    if not resource_by_district.empty:
    ...
    # SCHOOL SPONSOR ANALYSIS (Original Location)
    # st.header("🏛️ School Sponsor Analysis")
    # ...

    # SECTION 5: CRITICAL SCHOOLS (Original Location)
...
with tab2:
    ...
    st.markdown("Use this tab to compare key metrics between two districts, sponsors, and critical schools.")
    ...
    if d1 and d2 and d1 != d2:
        ...
        st.info("Please select two different districts to start a comparison.")
    ...
    tab_r1, tab_r2, tab_r3 = st.tabs(["👨‍🏫 Teachers", "🚽 Toilets", "🏫 Classrooms"])
